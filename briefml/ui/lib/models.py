...
class PredictionInput(BaseModel):
    ...
    choc_mode: int = Field(
        ..., ge=-1, le=9, description="Point de choc initial (-1 ou 0-9)"
    )
    driver_trajet_family: str = Field(..., description="Famille de trajet conducteur")
    time_bucket: str = Field(
        ...,
        description="Tranche horaire (night_00_05, morning_06_11, afternoon_12_17, evening_18_23)",  # noqa: E501
    )
    ...
    def validate_dep(cls, v: str) -> str:
        ...
        # Basic pattern check - full validation against ref_options.json happens server-side  # noqa: E501
        if not v or len(v) < 1:
...
class PredictionResult(BaseModel):
    ...
    probability: float = Field(
        ..., ge=0.0, le=1.0, description="Probabilité d'accident grave (0-1)"
    )
    ...
    inputs_summary: dict[str, Any] | None = Field(
        None, description="Résumé des entrées (pour vérification)"
    )
